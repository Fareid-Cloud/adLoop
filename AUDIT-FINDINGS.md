...
>
> وما يبقى بعد ذلك **ليس حاجزاً**: قراراتٌ تخصّك (رفعُ Next إلى ١٦، السحبُ التلقائيّ
> من البطاقة بعد تأكيد Paymob)، وتأكيداتٌ خارجية كانت موثَّقةً أصلاً في
...
|---|---|---|
| ١ | **رفع Next إلى ١٦** | آخِرُ ثغرتين (`postcss` وواحدة في `next`) لا تُغلَقان إلّا بترقيةٍ **كبرى** عبر ٤٧ صفحة. تستحقّ جولتَها ومراجعتَها، لا أن تركب مع رقعةٍ أمنية |
...
| ٣ | **نصُّ الشروط يعِد بالتجديد التلقائيّ** | (`terms.billingL2`) وهو غير مبنيّ. **قرارٌ لك:** إمّا السحبُ التلقائيّ بعد (٢)، أو تعديلُ النصّ ليطابق الواقع - وهو نصٌّ قانونيّ لا أعدّله بلا طلبك |
| ٤ | **E-1 العميق** | `cogs` نوعُه `number` لا `number \| null` في `pricingCalculator`. النصيحةُ الضارّة والحكمُ الكاذب أُغلِقا، ويبقى تحويلُ النوع عبر خمسة ملفّات - تنظيفٌ لا خطر |
| ٥ | **W-4 بوابات Scale** | قرارٌ واعٍ موثَّق في `CLAUDE.md`: التوسيعُ معلوماتيٌّ عمداً لأنّ الميزانية على مستوى المجموعة |
| ٦ | **S-5 · Paymob HMAC · مفتاح التكامل · تجربةُ استرداد نسخة · صلاحيات ميتا · حساب تيك توك · Sentry** | تأكيداتٌ خارجية موثَّقةٌ أصلاً في `docs/activation-checklist.md` |

...
export default async function CustomerDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 🔴 **الباقةُ الاتفاقية كانت مستثناةً من القائمة**، فمكانش فيه طريقٌ
  // في اللوحة كلّها تدّي حدّاً `enterprise` - والباقةُ دي **بتتمنح باليد
  // أصلاً وبس**: مافيش زرارُ شراء لها في المنتج لأنّ سعرها بالاتّفاق.
  // فاستثناؤها من مكان المنح الوحيد كان بيقفل الطريق الوحيد المفروض
  // يبقى مفتوح. (المسارُ نفسه كان بيقبلها من الأول - الواجهةُ وحدها هي
  // اللي مكانتش بتعرضها، وهو الشكلُ اللي بيتقري «الميزة مش موجودة».)
  const paidPlans = PLANS.filter((p) => p.key !== "free").map((p) => p.key);

...
const nextConfig = {
  ...
  turbopack: {},
  // 🔴 **`next dev` بيكتب في `CLAUDE.md` بنفسه.**
  //
  // بيلحق بلوكاً في آخر الملفّ (بين وسمَي `nextjs-agent-rules`) بيقول إنّ
  // الإصدار ده مختلفٌ عن اللي في تدريب النماذج ويوَدّي لـ`node_modules`.
  // والملفُّ ده **مكتوبٌ باليد وبيتحدّث بعناية** كإحاطةٍ للمشروع، وبيتحطّ
  // فيه قرارات معمارية والدروس اللي اتدفع تمنُها - فأداةٌ بتضيف فيه من
  // ورا صاحبه بتخلّي كلَّ `git status` يبان متّسخاً، وبتفتح البابَ لأن
  // يتكتب فوقه يوماً ما.
  //
  // والمحتوى اللي بيضيفه موجودٌ عندنا أصلاً وأدقّ: قسمُ «Next.js / بناء
  // المشروع» في `CLAUDE.md` بيسمّي `proxy.ts` وقيدَ `turbopack: {}`
  // وشرطَ `params: Promise` - وهي الحاجات اللي فعلاً وقعت البناء هنا.
  agentRules: false,
